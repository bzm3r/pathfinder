...
//type XlibDpy = *mut (*const c_void);// *mut ash::vk::Display;
//type XlibWindow = c_ulong; // ash::vk::Window;
//type XcbConnection = *mut (*const c_void);// *mut ash::vk::xcb_connection_t;
//type XcbWindow = u32; //*mut ash::vk::xcb_window_t;
//type WaylandDisplay = *mut c_void;
//type WaylandSurfacePointer = *mut c_void;
//type WaylandSize = u32;
//type WindowsInstance = *mut c_void;
//type WindowsHwnd = *mut c_void;
//type MetalLayer = *mut objc::runtime::Object;// Backend::window::CAMetalLayer;
//type MetalLayerEnableSignpost = bool;
//type MetalNSView = *mut c_void;
//
//#[cfg(all(unix, not(target_os = "android")), feature = "vulkan")]
//pub enum VulkanUnixWindow {
//    Xlib(XlibDpy, XlibWindow),
//    Xcb(XcbConnection, XcbWindow),
//    Wayland(WaylandDisplay, WaylandSurfacePointer, WaylandSize, WaylandSize),
//}
//
//#[cfg(feature = "metal")]
//pub enum MetalWindow {
//    FromLayer(MetalLayer, MetalLayerEnableSignpost),
//    FromNSView(MetalNSView),
//}
//
//#[cfg(all(unix, not(target_os = "android")), feature = "vulkan")]
//pub enum HalWindow {
//    #[cfg(all(unix, not(target_os = "android")), feature = "vulkan")]
//    VulkanUnix(VulkanUnixWindow),
//    #[cfg(windows, feature = "vulkan")]
//    VulkanMSWin(WindowsInstance, WindowsHwnd),
//    #[cfg(feature = "metal")]
//    Metal(MetalWindow),
//    #[cfg(windows, feature = "dx12")]
//    DX12(WindowsHwnd),
//    #[cfg(feature = "winit")]
//    Winit(&winit::window),
//}
...
impl HalDevice {
//    #[cfg(all(unix, not(target_os = "android")), feature = "vulkan")]
//    fn create_surface(instance: &Backend::Instance, window: HalWindow::VulkanUnix) -> &<Backend as hal::Backend>::Surface {
//        match window.0 {
//            VulkanUnixWindow::Xlib(xlib_dpy, xlib_window) => {
//                instance.create_surface_from_xlib(xlib_dpy, xlib_window)
//            },
//            VulkanUnixWindow::Xcb(connection, window) => {
//                instance.create_surface_from_xcb(connection, window)
//            },
//            VulkanUnixWindow::Wayland(display, surface, width, height) => {
//                instance.create_surface_from_wayland(display, surface, width, height)
//            }
//            VulkanUnixWindow::Windows(hinstance, hwnd) => {
//                instance.create_surface_from_hwnd(hinstance, hwnd)
//            }
//        }
//    }
//
//    #[cfg(windows, feature = "vulkan")]
//    fn create_surface(instance: &Backend::Instance, window: Hal::VulkanMSWin) -> &<Backend as hal::Backend>::Surface {
//        let (hinstance, hwnd) = window.0;
//        instance.create_surface_from_hwnd(hinstance, hwnd)
//    }
//
//    #[cfg(windows, feature = "dx12")]
//    fn create_surface(instance: &Backend::Instance, window: HalWindow::DX12) -> &<Backend as hal::Backend>::Surface {
//        instance.create_surface_from_hwnd(window.0)
//    }
//
//    #[cfg(feature = "metal")]
//    fn create_surface(instance: &Backend::Instance, window: HalWindow::Metal) -> &<Backend as hal::Backend>::Surface {
//        match window.0 {
//            MetalWindow::FromLayer(layer, enable_signposts) => {
//                instance.create_surface_from_layer(layer, enable_signposts)
//            },
//            MetalWindow::FromNSView(nsview) {
...
//        let mut surface = if cfg!(feature = "winit") {
//            match window {
//                HalWindow::Winit(w) => {
//                    instance.create_surface(w)
//                },
//                _ => {
//                    panic!("Winit is enabled, but a winit window was not given through HalWindow!");
//                }
//            }
//        } else {
//            HalDevice::create_surface(instance, window)
//        };
    ...
    fn create_swap_chain<W: Window>(
        ...
        let extent = {
            let window_client_area = window.get_client_area();
